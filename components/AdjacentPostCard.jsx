...
const AdjacentPostCard = ({ post, position }) => (
    <>
        ...
        <Link href={`/post/${post.slug}`}><span className="z-10 cursor-pointer absolute w-full h-full justify-items-center" /></Link>
        {position === 'LEFT' && (
            <div className="absolute arrow-btn bottom-5 text-center py-3 cursor-pointer bg-pink-600 left-4 rounded-full">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-12 text-white object-center" fill="none" viewBox="0 0 24 24" stroke="currentColor">
